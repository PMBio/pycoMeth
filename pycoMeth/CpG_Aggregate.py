...
def CpG_Aggregate(
    ...
    * min_llr
    ...
    with CpG_Writer(
        bed_fn=output_bed_fn, tsv_fn=output_tsv_fn, sample_id=sample_id, min_llr=min_llr, verbose=verbose
    ) as fp_out:
        for coord, val_dict in tqdm(
            sites_index, unit=" sites", unit_scale=True, desc="\tProgress", disable=not progress
        ):
...
class SitesIndex:
    ...
    def add(self, lt):
        ...
        if chrom not in self.sites:
            ...
            if (
                not pos in self.sites[chrom]
            ):  ####################################################### Could also collect read_name, log_lik_methylated, log_lik_unmethylated
                self.counter["Initial Sites"] += 1
                self.sites[chrom][pos] = {"sequence": lt.sequence, "num_motifs": lt.num_motifs, "llr": [], "n_reads": 0}
